...
struct Args : MainArguments<Args> {
    ...
    std::vector<std::string> segmentAction =
        option("segment_action", 'a', "recording segmenting action, e.g. left,trigger,click")
        .validator([](auto v) { return v.size() == 3 || v.size() == 0; });
    std::vector<std::string> startAction =
        option("start_action", 'w', "recording starting action, e.g. left,trigger,click, can be same as segmentation action")
        .validator([](auto v) { return v.size() == 3 || v.size() == 0; });
...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < vr::k_unMaxTrackedDeviceCount; ++i)
    {
        ...
        if (deviceClass == vr::TrackedDeviceClass_HMD)
        {
            ...
            {
                hardwareData.m_captureFrequency = 
                    2.0f * hardwareData.m_hmd.m_device.m_floatProperties[vr::Prop_DisplayFrequency_Float].m_value;
            }
        }
        else
        {
...
void initVR()
{
  ...
  if (hmdErr != vr::EVRInitError::VRInitError_None)
  {
    LOGE("Failed to initialize: %s %s \n\n",
      vr::VR_GetVRInitErrorAsSymbol(hmdErr),
...
void printDeviceInfo(const vr::TrackedDeviceIndex_t deviceId)
{
  ...
  std::stringstream ss;
  ss << "\nDevice " << deviceId << " Class " << deviceClass
    << " / " << magic_enum::enum_name(deviceClass) << "\n";
...
void collectDeviceProperties(const vr::TrackedDeviceIndex_t deviceId, VRData::Device& device)
{
  ...
  for (uint32_t p = 1; p < 20000; ++p) {  // Can't take Prop_TrackedDeviceProperty_Max here, takes too long
    ...
    { // string
      ...
      if (error == vr::TrackedProp_Success) {
        device.m_stringProperties[propertyId] =
        {
            std::string(magic_enum::enum_name(propertyId)),
            std::string(buf.get())
        };
        continue;
      }
    ...
    { // int32
      ...
      if (error == vr::TrackedProp_Success) {
        device.m_int32Properties[propertyId] =
        {
            std::string(magic_enum::enum_name(propertyId)),
            valueInt32
        };
    ...
    { // uint64
      ...
      if (error == vr::TrackedProp_Success) {
        device.m_uint64Properties[propertyId] =
        {
            std::string(magic_enum::enum_name(propertyId)),
            valueUint64
        };
    ...
    { // bool
      ...
      if (error == vr::TrackedProp_Success) {
        device.m_boolProperties[propertyId] =
        {
            std::string(magic_enum::enum_name(propertyId)),
            valueBool
        };
    ...
    { // float
      ...
      if (error == vr::TrackedProp_Success) {
        device.m_floatProperties[propertyId] =
        {
            std::string(magic_enum::enum_name(propertyId)),
            valueFloat
        };
...
void setupActions(std::string modelNumber, VRData::Hand hand)
{
  ...
  {
    actions = {
        {VRData::ActionType::DIG, "system", "click", hand},
  ...
  else if (modelNumber.find("Oculus") != std::string::npos)
  {
    actions = {
        ...
        {VRData::ActionType::DIG, "trigger", "touch", hand},
        {VRData::ActionType::VEC1, "trigger", "value", hand},
        {VRData::ActionType::VEC1, "grip", "value", hand},
        {VRData::ActionType::DIG, "grip", "touch", hand},
        {VRData::ActionType::DIG, "joystick", "click", hand},
        {VRData::ActionType::DIG, "joystick", "touch", hand},
  ...
  for (const auto& a : actions)
  {
    ...
    {
      ss << "Error getting handle for action "
        << action.name << "_" << action.input << ": " << err << " / "
...
std::string setupStartAction()
{
    ...
    else
    {
        LOGE("\tError getting handle for start action %s_%s: %i / %s\n",
            startAction.name.c_str(), startAction.input.c_str(),
            err, std::string(magic_enum::enum_name(err)).c_str());
    }
...
void setupSegmentationAction()
{
    ...
    else
    {
        LOGE("\tError getting handle for segmenting action %s_%s: %i / %s\n",
            segmentAction.name.c_str(), segmentAction.input.c_str(), 
            err, std::string(magic_enum::enum_name(err)).c_str());
    }
}
...
void notifyHMD(std::string message, uint32_t overrideTime)
{
  ...
  vr::EVRNotificationError nerr = vrNotifications->CreateNotification(
    overlayHandle, 42, vr::EVRNotificationType_Transient,
    message.c_str(), vr::EVRNotificationStyle_None,
    messageIconPtr, &notificationId
  );
